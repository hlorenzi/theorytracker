...
export function fillStyleForDegree(ctx, degree)
{
    degree = mod(degree, 7)
	...
	const colorBefore = Theory.Utils.degreeToColor(mod(Math.floor(degree), 7))
	const colorAfter  = Theory.Utils.degreeToColor(mod(Math.ceil(degree), 7))
...
export function drawChord(ctx, x1, y1, x2, y2, chord, key)
{
	const w = x2 - x1
	const h = y2 - y1
	const decorH = 6
