	...
	for (var i = 0; i < this.viewBlocks.length; i++)
	{
		...
		for (var n = block.meter.tick - block.tick; n < block.duration; n += this.WHOLE_NOTE_DURATION / block.meter.denominator)
		{
			if (n >= 0)
			{
				...
				this.ctx.moveTo(block.x1 + n * this.tickZoom, block.y2 + this.CHORDNOTE_MARGIN);
				this.ctx.lineTo(block.x1 + n * this.tickZoom, block.y2 + this.CHORDNOTE_MARGIN + this.CHORD_HEIGHT);
				this.ctx.stroke();
			}
