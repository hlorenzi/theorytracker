...
export class Instrument
{
	...
	constructor(synth: Synth, sampleDefs: [number, string][])
	{
		this.samples = []
	...
	load(synth: Synth, url: string, frequency: number)
	{
		let sample =
		{
	...
	playNote(synth: Synth, desiredFreq: number, desiredVolume: number, duration: number)
	{
		...
		// Play the samples.
		const voices = [ this.playVoice(synth, this.samples[sample1], desiredFreq, desiredVolume * volume1) ]
		if (sample2 != null)
			voices.push(this.playVoice(synth, this.samples[sample2], desiredFreq, desiredVolume * volume2))
	...
	playVoice(synth: Synth, sample: any, frequency: number, volume: number)
	{
		let sourceNode = synth.audioCtx.createBufferSource()
		sourceNode.playbackRate.value = frequency / sample.frequency
		sourceNode.buffer = sample.buffer
		sourceNode.start(0)
		
		let envelopeNode = synth.audioCtx.createGain()
		envelopeNode.gain.setValueAtTime(1, synth.audioCtx.currentTime)
		
		let volumeNode = synth.audioCtx.createGain()
		volumeNode.gain.value = volume
		
		sourceNode.connect(envelopeNode)
		envelopeNode.connect(volumeNode)
		volumeNode.connect(synth.audioCtxOutput)
		
		return { source: sourceNode, envelope: envelopeNode, volume: volumeNode }
	}


	processNote(synth: Synth, noteData: any, deltaTime: number)
	{
		...
		if (noteData.duration <= 0 && !noteData.released)
		{
			...
			{
				noteData.voices[i].envelope.gain.setValueAtTime(1, synth.audioCtx.currentTime)
				noteData.voices[i].envelope.gain.linearRampToValueAtTime(0, synth.audioCtx.currentTime + releaseFalloffDuration)
			}
		}
	...
	stopNote(synth: Synth, noteData: any)
	{
		for (let i = 0; i < noteData.voices.length; i++)
			noteData.voices[i].source.stop()
	}
